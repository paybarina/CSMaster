...
Java Developer
Created on 3/6/2023 1:19 AM
@Last Modified 3/6/2023 1:19 AM
Version 1.0
*/
...
public class CategoryController {
    ...
    // handler method to handle list category and return mode and view
    @GetMapping("/v1/category")
    public String listCategory(Model model, WebRequest request) {
    ...
    public String saveCategory(@ModelAttribute("category") Category category,Model model,WebRequest request) {
        ...
        return "redirect:/api/category/v1/category";
    }

    @GetMapping("/v1/category/edit/{id}")
    public String editCategoryForm(@PathVariable Long id, Model model,WebRequest request) {
        if(OtherConfig.getFlagSessionValidation().equals("y")) {
            mappingAttribute.setAttribute(model, objectMapper, request);//untuk set session
            if (request.getAttribute("USR_ID", 1) == null) {
                return "redirect:/api/check/logout";
    ...
    public String updateCategory(@PathVariable Long id,
        ...
        // get categoryfrom database by id
